...
class Runner {
  ...
  async init(deploymentSigner?: Signer): Promise<this> {
    ...
    const accountDeployer2 = DeterministicDeployer.getAddress(
      new SimpleAccountFactory2__factory(),
      0,
      [this.entryPointAddress],
    );
    ...
    if (!(await dep.isContractDeployed(accountDeployer))) {
      ...
      const dep2 = new DeterministicDeployer(
        deploymentSigner.provider as any,
        deploymentSigner,
      );

      await dep2.deterministicDeploy(new SimpleAccountFactory2__factory(), 0, [
        this.entryPointAddress,
      ]);
  ...
  async runUserOp(target: string, data: string): Promise<void> {
    ...
    console.log(
      "Account API 2 CC",
      await this.accountApi2?.getCounterFactualAddress(),
    );
    ...
    console.log(
      "Account API 1 CC",
      await this.accountApi?.getCounterFactualAddress(),
    );
  }
