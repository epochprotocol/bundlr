...
} from "./interfaces/IAdvancedUserOperation";
...
export class HttpRpcClient {
  ...
  async sendUserOpToBundler(
    ...
    const trx = await this.userOpJsonRpcProvider.send("eth_sendUserOperation", [
      hexifiedUserOp,
      this.entryPointAddress,
    ]);
    console.log(trx);
  ...
  async getUserOperations(
    address: string,
  ): Promise<Array<IAdvancedMempoolEntry>> {
    const mempoolEntry: Array<IAdvancedMempoolEntry> =
      await this.userOpJsonRpcProvider.send("eth_getUserOperations", [address]);
