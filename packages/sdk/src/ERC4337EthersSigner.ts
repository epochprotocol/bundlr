...
import { AdvancedTransactionStruct } from './interfaces/IAdvancedTransaction'
...
export class ERC4337EthersSigner extends Signer {
  ...
  async sendTransaction(transaction: Deferrable<AdvancedTransactionStruct>): Promise<TransactionResponse> {
    const tx: AdvancedTransactionStruct = await this.populateTransaction(transaction)
    ...
    const userOperation = await this.smartAccountAPI.createSignedUserOp({
      ...
      advancedUserOperation: await transaction.advancedUserOperation,
